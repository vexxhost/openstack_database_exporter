CREATE TABLE
    `agents` (
        ...
        `resources_synced` tinyint (1) DEFAULT NULL,
        UNIQUE KEY `uniq_agents0agent_type0host` (`agent_type`, `host`),
        KEY `ix_agents_host` (`host`)
    );
...
CREATE TABLE
    `securitygrouprules` (
        ...
        `port_range_min` int DEFAULT NULL,
        `port_range_max` int DEFAULT NULL,
        `remote_ip_prefix` varchar(255) DEFAULT NULL,
        `standard_attr_id` bigint NOT NULL,
        `remote_address_group_id` varchar(36) DEFAULT NULL,
        `normalized_cidr` varchar(255) DEFAULT NULL
    );
