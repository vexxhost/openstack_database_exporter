...
const GetProviderAllocations = `-- name: GetProviderAllocations :many
SELECT
    rp.name as hostname,
...
type GetProviderAllocationsRow struct {
...
func (q *Queries) GetProviderAllocations(ctx context.Context) ([]GetProviderAllocationsRow, error) {
	rows, err := q.db.QueryContext(ctx, GetProviderAllocations)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetProviderAllocationsRow
	for rows.Next() {
		var i GetProviderAllocationsRow
		if err := rows.Scan(
			&i.Hostname,
			&i.Uuid,
			&i.ResourceType,
