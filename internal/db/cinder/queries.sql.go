...
const GetAllProjectQuotas = `-- name: GetAllProjectQuotas :many
SELECT
    q.project_id,
    q.resource,
    q.hard_limit,
...
func (q *Queries) GetAllProjectQuotas(ctx context.Context) ([]GetAllProjectQuotasRow, error) {
	rows, err := q.db.QueryContext(ctx, GetAllProjectQuotas)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetAllProjectQuotasRow
	for rows.Next() {
		var i GetAllProjectQuotasRow
		if err := rows.Scan(
			&i.ProjectID,
			&i.Resource,
			&i.HardLimit,
...
const GetDefaultQuotaLimits = `-- name: GetDefaultQuotaLimits :many
SELECT
    resource,
    hard_limit
FROM
...
type GetDefaultQuotaLimitsRow struct {
	Resource  string
	HardLimit sql.NullInt32
}

func (q *Queries) GetDefaultQuotaLimits(ctx context.Context) ([]GetDefaultQuotaLimitsRow, error) {
	rows, err := q.db.QueryContext(ctx, GetDefaultQuotaLimits)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetDefaultQuotaLimitsRow
	for rows.Next() {
		var i GetDefaultQuotaLimitsRow
...
func (q *Queries) GetVolumeTypes(ctx context.Context) ([]GetVolumeTypesRow, error) {
	rows, err := q.db.QueryContext(ctx, GetVolumeTypes)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetVolumeTypesRow
	for rows.Next() {
		var i GetVolumeTypesRow
